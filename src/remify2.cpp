...
// include functions from remify.cpp (only keep functions really needed; for later)
#include "remify_helpers.h"
...
// @title getOmitDyadActiveRiskSet2 (omit_dyad has different format, where indices which dyads are included)
//
...
//
Rcpp::List getOmitDyadActiveRiskSet2(std::string model,
                                     arma::uvec actor1,
                                     arma::uvec actor2,
                                     arma::uvec type,
                                     int D,
  ...
  // Collect dyads that occur at least once
  std::vector<int> active_dyads_vec;
  active_dyads_vec.reserve(M);

// #ifdef _OPENMP
//   omp_set_dynamic(0);
//   omp_set_num_threads(ncores);
// #pragma omp parallel for if(ncores>1) private(m) shared(M,actor1,actor2,type,N,directed,active_dyads_vec)
// #endif
  for (m = 0; m < M; m++) {
    int dyad_m = remify::getDyadIndex(actor1(m)-1, actor2(m)-1, type(m)-1, N, directed);
    active_dyads_vec.push_back(dyad_m);
  }

  // Sort + unique (0-based dyad indices)
  ...
  Rcpp::List out = Rcpp::List::create(
    Rcpp::Named("time") = which_time,
    Rcpp::Named("riskset_idx") = active_dyads + 1, // 1-based for R
    Rcpp::Named("D_active") = active_dyads.n_elem
  );
  ...
  // Map each event to its index in active_dyads
...
Rcpp::List getOmitDyadManualRiskSet3(std::string model,
                                     ...
                                     arma::uvec actor2,
                                     arma::uvec type,
                                     Rcpp::DataFrame manual_riskset,
                                     const std::vector<std::string>& actorName,
  ...
  for (arma::uword i = 0; i < K; ++i) {
    ...
    auto it1 = std::find(actorName.begin(), actorName.end(), a1s);
    auto it2 = std::find(actorName.begin(), actorName.end(), a2s);

    if (it1 == actorName.end() || it2 == actorName.end()) {
...
// @title getRisksetIdx (sparse replacement for getRiskset)
//
// @return List of length Z; each element is an IntegerVector of
//         1-based dyad indices INCLUDED in the riskset
Rcpp::List getRisksetIdx(Rcpp::List which_dyad,
                         int C,
                         int D,
                         int N,
                         bool directed) {
  ...
  // excluded dyads per z (temporary)
  std::vector< std::vector<int> > excluded(Z);
  ...
  for (z = 0; z < Z; z++) {
    ...
    for (d = 0; d < D_z; d++) {
      ...
      if (type_na(d)) { // [?,?,NA]
        if (!actor1_na(d) && !actor2_na(d)) { // [X,Y,NA]
          for (c = 0; c < C; c++) {
            if (actor1_z(d) != actor2_z(d)) {
              int dyad = remify::getDyadIndex(actor1_z(d), actor2_z(d), c, N, directed);
              excluded[z].push_back(dyad);
            }
          }
        } else if (!actor1_na(d)) { // [X,NA,NA]
          for (c = 0; c < C; c++) {
            for (j = 0; j < N; j++) {
              if (j != actor1_z(d)) {
                int dyad = remify::getDyadIndex(actor1_z(d), j, c, N, directed);
                excluded[z].push_back(dyad);
              }
            }
          }
          ...
          for (c = 0; c < C; c++) {
            for (i = 0; i < N; i++) {
              if (i != actor2_z(d)) {
                int dyad = remify::getDyadIndex(i, actor2_z(d), c, N, directed);
                excluded[z].push_back(dyad);
              }
            }
          }
        ...
        if (actor1_na(d) && actor2_na(d)) { // [NA,NA,C]
          for (i = 0; i < N; i++) {
            for (j = 0; j < N; j++) {
              if (i != j) {
                int dyad = remify::getDyadIndex(i, j, type_z(d), N, directed);
                excluded[z].push_back(dyad);
              }
            }
          }
        } else if (!actor1_na(d) && !actor2_na(d)) { // [X,Y,C]
          excluded[z].push_back(
              remify::getDyadIndex(actor1_z(d), actor2_z(d), type_z(d), N, directed)
          );
        } else if (!actor1_na(d)) { // [X,NA,C]
          for (j = 0; j < N; j++) {
            if (j != actor1_z(d)) {
              int dyad = remify::getDyadIndex(actor1_z(d), j, type_z(d), N, directed);
              excluded[z].push_back(dyad);
            }
          }
        } else { // [NA,Y,C]
          for (i = 0; i < N; i++) {
            if (i != actor2_z(d)) {
              int dyad = remify::getDyadIndex(i, actor2_z(d), type_z(d), N, directed);
              excluded[z].push_back(dyad);
            }
          }
        }
  ...
  // Build included indices per z
  Rcpp::List out(Z);
  for (z = 0; z < Z; z++) {
    std::vector<char> mask(D, 1);        // local, freed per z
    for (int dy : excluded[z]) mask[dy] = 0;

    std::vector<int> included;
    for (int d0 = 0; d0 < D; d0++) {
      if (mask[d0]) included.push_back(d0 + 1); // 1-based for R
    }
    out[z] = Rcpp::wrap(included);
  }
...
Rcpp::List processOmitDyadIdx(Rcpp::List convertedOmitDyad, Rcpp::List convertedOmitDyad_time, arma::uword M, int C, int D, int N, bool directed, std::string model) {
    ...
    // (3.3) create sparse changing riskset per element of which_dyad
    Rcpp::List riskset_idx = getRisksetIdx(which_dyad, C, D, N, directed);

    // arrange output in a list
    Rcpp::List out = Rcpp::List::create(
      Rcpp::Named("time") = which_time,
      Rcpp::Named("riskset_idx") = riskset_idx
    );
...
Rcpp::List convertInputREHIdx( Rcpp::DataFrame input_edgelist,
...
Rcpp::List convertInputREHIdx3(
  ...
  Rcpp::List omit_dyad_list;
  if (Rf_isNull(omit_dyad)) {
    omit_dyad_list = Rcpp::List();     // empty => length 0
  } else {
    omit_dyad_list = Rcpp::as<Rcpp::List>(omit_dyad);
  }
...
#pragma omp parallel for if(ncores>1) private(m) shared(M,stringActor1,stringActor2,actorName,actorID,convertedActor1_ID,convertedActor2_ID,dyad,time_loc)
  ...
  if (has_manual_riskset) {
    ...
    Rcpp::DataFrame mr = Rcpp::as<Rcpp::DataFrame>(manual_riskset);

    arma::uvec a1 = Rcpp::as<arma::uvec>(out["actor1_ID"]);
    arma::uvec a2 = Rcpp::as<arma::uvec>(out["actor2_ID"]);

    Rcpp::List outOmitDyad = getOmitDyadManualRiskSet3(
      model,
      a1, a2, type_loc,
      mr,
      actorName, typeName,
  ...
  else if(omit_dyad_list.size() > 0){
    // input time
    std::vector<double> time = Rcpp::as<std::vector<double>>(convertedEdgelist["time"]); // consider the vector of time from the convertedEdgelist (because there might be some self-loop removed, in turn, changing the size of the time vector)
    Rcpp::List convertedOmitDyad = Rcpp::List::create(); // r-th list with matrix inputs converted into IDs
    Rcpp::List convertedOmitDyad_time = Rcpp::List::create(); // r-th list with time inputs converted into IDs
    int N = actorName.size();
    R = omit_dyad_list.length();
    int M_processed = time.size();

    for(r = 0; r < R; r++){
      // converting r-th element in omit_dyad
      Rcpp::List omit_r = omit_dyad_list[r]; // r-th input of `omit_dyad`
...
// @title remifyCpp2 (the Rcpp alias of \code{remify2()})
//
...
Rcpp::List remifyCpp3(
  ...
  // Build the actor universe used for the actor dictionary.
  // If manual_riskset is provided: use (edgelist actors) ∪ (manual_riskset actors), ignore `actors`.
  // Else: original behavior (edgelist actors plus optional `actors` argument).

  Rcpp::StringVector actor1_and_actor2;

  if (has_manual_riskset) {

    Rcpp::DataFrame mr = Rcpp::as<Rcpp::DataFrame>(manual_riskset);

    if (!mr.containsElementNamed("actor1") || !mr.containsElementNamed("actor2")) {
      Rcpp::stop("`manual_riskset` must contain columns `actor1` and `actor2`.");
    }

    Rcpp::StringVector mr_actor1 = Rcpp::as<Rcpp::StringVector>(mr["actor1"]);
    Rcpp::StringVector mr_actor2 = Rcpp::as<Rcpp::StringVector>(mr["actor2"]);

    actor1_and_actor2 = Rcpp::StringVector(
      actor1.length() + actor2.length() + mr_actor1.length() + mr_actor2.length()
    );

    arma::uword idx = 0;

    actor1_and_actor2[Rcpp::Range(idx, idx + actor1.length() - 1)] = actor1;
    idx += actor1.length();

    actor1_and_actor2[Rcpp::Range(idx, idx + actor2.length() - 1)] = actor2;
    idx += actor2.length();

    actor1_and_actor2[Rcpp::Range(idx, idx + mr_actor1.length() - 1)] = mr_actor1;
    idx += mr_actor1.length();

    actor1_and_actor2[Rcpp::Range(idx, idx + mr_actor2.length() - 1)] = mr_actor2;

  } else {

    // Original behavior: include optional `actors` argument
    arma::uword actors_vector_length = 0;
    Rcpp::StringVector actors_vector;
    ...
    actor1_and_actor2 = Rcpp::StringVector(
      actor1.length() + actor2.length() + actors_vector_length
    );

    actor1_and_actor2[Rcpp::Range(0, actor1.length() - 1)] = actor1;
    actor1_and_actor2[Rcpp::Range(actor1.length(),
                                  actor1.length() + actor2.length() - 1)] = actor2;

    if (!Rf_isNull(actors)) {
      actor1_and_actor2[Rcpp::Range(actor1.length() + actor2.length(),
                                    actor1_and_actor2.length() - 1)] = actors_vector;
    }
  }
...
//
// [[Rcpp::export]]
Rcpp::List remify2relventremIdx(arma::vec actor1,
                            arma::vec actor2,
                            arma::vec type,
                            arma::vec dyad,
                            arma::uword M,
    ...
    if (omit_dyad.size() > 1) {
      ...
      omp_set_dynamic(0);
      omp_set_num_threads(ncores);
#pragma omp parallel for if(ncores>1) private(m) shared(M,omit_dyad_time,supplist,rsi,D)
#endif
      for (m = 0; m < M; m++) {
