...
<i>This vignette explains aim, input, output, attributes and methods of the function <tt>remify::reh()</tt>.</i>
...
### <b><tt>edgelist</tt></b>
`edgelist` is a `data.frame` and consists of the converted input edgelist, with columns `[time,actor1_ID,actor2_ID,type_ID,weight]` where the inputs `actor1`, `actor2` and `type` are converted into IDs. If `weight` is not supplied as input, then its corresponding column in the output edgelist will be a vector of $1$'s.
