...
ls(envir = .GlobalEnv) # objects loaded with 'randomREH'
```
...
```{r}
head(omit_dyad[[1]]$time) # printing out only a few time points
```
...
```{r}
head(omit_dyad[[2]]$time) # printing out only a few time points
``` 
...
```{r}
summary(edgelist_reh) # code to be changed
```
...
# first time window (from the 1 time point to the 3999th)
