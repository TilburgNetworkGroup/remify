...
<i>This vignette explains aim, input, output, attributes and methods of the function `<tt>`remify::remify()`.</i>
...
### <b><tt>model</tt></b>
Whether the model of interest is tie-oriented or actor-oriented, the argument `model` can be specified either as `"tie"` or as `"actor"`. In the case of an actor-oriented model with changing risk set, the output will consist of two risk sets: senders' and dyads' risk sets. For a tie-oriented modeling with changing risk set the output object will contain only the riskset of the dyads. If no dynamic risk set is defined (`omit_dyad = NULL`), the argument `model` won't affect the output object.
...
### <b><tt>M</tt></b>
`M` is the number of observed time points. If there are events occurring at the same time point, then the time point will count only once. If all events occurred at different time points, then `M` will correspond also to the number of events.
...
### <b><tt>D</tt></b>
`D` is the number of dyads considering also the event types (if measured). The number of dyads changes based on the value of the `riskset` argument :

  - If `riskset =  "full"`
    * and the network is <i>directed</i>, then $D = N*(N-1)*C$;
    * and the network is <i>undirected</i>, then $D = (N*(N-1)/2)*C$.
  - if `riskset = "active"`, then $D$ is the number of active dyads observed in the event history
...
### <b><tt>edgelist</tt></b>
`edgelist` is a `data.frame` and consists in the converted input edgelist, with columns `[time,actor1_ID,actor2_ID,type_ID,weight]` where actors and types supplied in the input edgelist are converted into IDs. If `type` or `weight` are not supplied as input, then the output edgelist will not incude their columns.
...
### <b><tt>dyad</tt></b>
`dyad` is the vector of IDs as to the observed dyads. The IDs range between 1 and `D` (`edgelist_reh$D`). This attribute is empty if `model = "actor"`. For more details about how the ID is assigned to a specific triple of `[actor1,actor2,type]`, see `vignette(topic = "riskset", package = "remify")`.
...
`dyadIDactive` is the vector of relational events that follows the dyadID's of the active set of dyads. This attribute is available only when `riskset = "active"`.

```{r}
attr(edgelist_reh, "dyadIDactive")
```
...
### <b><tt>dim()</tt></b>
`dim()` returns some useful dimensions characterizing the network, such as: number of events, number of actors, number of event types (omitted in the case of networks with one or no event types), largest number of possible dyads given the number of actors ($N$). 
...
### <b><tt>plot()</tt></b>
`plot()` returns a sequence of three plots:

1. distribution of the waiting times
2. activity plot (tile plot on dyad count, with marginal in-degree and out-degree distribution of actors, if a directed network)
