...
## Aim
The objective of <tt>remify::remify()</tt> is to process raw relational event data provided by the user along with other inputs that characterize them (actors' names, event types' names, starting time point of the event sequence, set of interactions to be excluded from the risk set at specific time points, etc.).
...
<b>Input</b> arguments that can be supplied to `remify()` are: `edgelist`, `actors`, `types`, `directed`, `ordinal`, `origin`, `omit_dyad` and `model`. 
...
### <b><tt>C</tt></b>
`C` is the number of event types (also referred as the sentiment of the event) that could be observed in the network. If no event types are present in the network or only one event type is defined, then the output in `C` will be `NULL`
...
### <b><tt>edgelist</tt></b>
`edgelist` is a `data.frame` and consists of the converted input edgelist, with columns `[time,actor1_ID,actor2_ID,type_ID,weight]` where the inputs `actor1`, `actor2` and `type` are converted into IDs. If `type` or `weight` are not supplied as input, then the output edgelist will not incude them columns.
...
If `omit_dyad` is supplied to the processing function, then the `remify()` output object will contain the processed list under the same name. In the case of tie-oriented modeling, the list consists of two objects: a vector named `time` and a matrix named `riskset`.
...
The processed object `omit_dyad` (inside any `remify` object) will be required by other packages in [<tt>remverse</tt>](https://github.com/TilburgNetworkGroup/remverse). Given that such packages have function written in C++, the row indices in the vector start at 0 (indicating row 1 in the matrix above) and they assume value -1 when no risk set alteration is observed.
...
Whenever `omit_dyad` is not supplied to `reh()` then its output in the `remify` object will be an empty list.
...
`dictionary` is a list of two `data.frame`'s: `actors` and `types`.
  ...
  - `actors` has two columns: the first with actor names (`actorName`) sorted according to their alphanumerical order, the second with their corresponding ID's (`actorID`), ranging in $\left\lbrace 1,\ldots,N \right\rbrace$;
  - `types` has two columns: the first with type names (`typeName`) sorted according to their alphanumerical order, the second with their corresponding ID's (`typeID`), ranging in $\left\lbrace 1,\ldots,C \right\rbrace$; 

```{r}
attr(edgelist_reh, "dictionary")
```

For more details about how the ID is assigned to actors and types, see `vignette("riskset","remify")`
...
### <b><tt>dyad</tt></b>
`dyad` is the vector of IDs as to the observed dyads. The IDs range between 1 and `D` (`edgelist_reh$D`). This attribute is empty if `model = "actor"`. For more details about how the ID is assigned to a specific triple of `[actor1,actor2,type]`, see `vignette("riskset","remify")`.

```{r}
attr(edgelist_reh, "dyad")[1:10] # printing out only first 10 observed dyads
```


## Methods

The available <b>methods</b> for a `remify` object are: `print`, `summary`, `dim`, `getDynamicRiskset`, `getActorName`, `getTypeName`, `getDyad`, `getActorID`, `getTypeID`, `getDyadID` and `plot`.

------

### <b><tt>print() and summary()</tt></b>
Both `print(reh)` and `summary(reh)` print out a brief summary of the relational network data.

```{r}
summary(edgelist_reh) # same output as `print(reh)` or just `reh`
```

------

### <b><tt>dim()</tt></b>
`dim(reh)` returns some useful dimensions characterizing the network, such as: number of events, number of actors, number of event types (omitted in the case of network with one or no event types), number of dyads. 
...
If `omit_dyad` is supplied to the processing function, `getDynamicRiskset(reh)` returns the processed dynamic risk set matrix (or the two dynamic risk sets if actor-oriented modeling) that are explained in the 'Output' section under the section about `omit_dyad`.

```{r}
getDynamicRiskset(x = edgelist_reh)$riskset[,1:10] # printing out the risk set modifications of only the first 10 columns (dyads). A total number of 2 modifications of the risk set are observed (by row)
```
------
...
### <b><tt>getDyad()</tt></b>
`getDyad(x, dyadID)`: by supplying one or more `dyadID`'s (between `1` and `edgelist_reh$D`) it returns a `data.frame` with the dyad composition as "actor1", "actor2" and "type" (if type is present).

```{r}
getDyad(x = edgelist_reh, dyadID = c(1,10,100))
```
...
### <b><tt>plot()</tt></b> _$\cdot$ not fully working yet $\cdot$_
`plot(x,...)` returns a sequence of three plots:

1. distribution of the waiting times
2. activity plot (tile plot on dyad count, with marginal in-degree and out-degree distribution of actors)
3. descriptive dyad and actor activity rate per time unit specified by the user (also calculated per event type, if type is present)


```{r}
# plot(x = edgelist_reh, x,which = c(1:4),caption = list("Title 1","Title 2","Title 3","Title 4"),APA_style = FALSE, color_palette = "BuPu")
```
